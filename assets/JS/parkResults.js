...
//Function to check for and pull saved search history
document.addEventListener('DOMContentLoaded', function () {
...
// Function to display the search history on the web page
function displaySearchHistory() {
  const historyContainer = document.getElementById('search-history')
  historyContainer.innerHTML = '<p>Search History:</p>'

  for (let i = 0; i < searchHistory.length; i++) {
    const city = searchHistory[i]
    const historyItem = document.createElement('p')
    historyItem.textContent = `${i + 1}. ${city}`
    historyContainer.appendChild(historyItem)
  }
}

// Add an event listener to the search button to update the search history
searchBtn.addEventListener('click', function () {
  const userLocationInput = document.getElementById('find_parks').value
  if (userLocationInput.trim() !== '') {
    addToSearchHistory(userLocationInput)
  }
  fetchParkNames()
})
