...
function fetchWeatherAndForecast(lat, lon, callback) {
  ...
  fetch(weatherUrl)
    ...
    .then(([forecastResponse, formattedWeatherInfo]) => { //process weather data and builds HTML string to display 5 day forecast
...
function buildFeeInfoHTML(entranceFees) {
  ...
  entranceFees.forEach((entranceFees) => { // Loop through each entrance fee item and append its information to the HTML string.
    feeInfoHTML += `
      <div>
...
function geocodeLocation(location) {
  ...
  return fetch(geocodeApiUrl)
    ...
    .then(function (data) {  // Extract the coordinates from the geocode data.
...
function searchParksWithinRadius(allParks) {
  ...
  if (userLocationInput !== '' && userLocationInput !== null) { // Check if the user has entered a location. If not, display all parks.
    if (!selectedRadius) { // If no radius is selected, display all parks without filtering by radius.
      buildCards(allParks)
      return
    }
    geocodeLocation(userLocationInput) // Call the geocodeLocation function to convert the user's location input into geographic coordinates.
      .then(function (userCoordinates) {
        if (!userCoordinates) {
          ...
          var filteredParks = allParks.filter(function (park) {     // Filter the list of all parks to find those within the selected radius of the user's location.
            var parkCoordinates =
              park.latitude && park.longitude
                ? { lat: park.latitude, lng: park.longitude }
                : null

            if (  // If the park has coordinates and is within the selected radius of the user's location, include it in the filtered list.
              parkCoordinates &&
              calculateDistance(
                userCoordinates.lat,
                userCoordinates.lng,
